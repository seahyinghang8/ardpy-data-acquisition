...
class VoltageThread(threading.Thread):
    ...
    def run(self):
        ...
        if self._arduino.readline():
            ...
            wx.CallAfter(pub.sendMessage, "show_message", message="Port %s is not functioning. Check if arduino is properly functioning." %self.port)
        ...
        while self._continue and self._arduino.isOpen():
            ...
            if self._arduino.isOpen():
                ...
                volt = "%.2f" %values[0]
                diff = "%.2f" %(values[1] - values[2])

                # publisher to send to update the voltage
                wx.CallAfter(pub.sendMessage, "update_voltage", volt=volt, diff=diff)
            else:
                # clear the current field
                wx.CallAfter(pub.sendMessage, "update_voltage", volt="-", diff="-")
...
class MainFrame(gui.FrameMain):
    ...
    def updateVoltage(self, volt, diff):
        self.voltageDisplay.SetValue(volt)
        self.differenceDisplay.SetValue(diff)
    ...
    def selectChart(self, event):
        if self.chartChoice.GetCurrentSelection() == 0:
            self.text6.Show()
            self.differenceDisplay.Show()
        else:
            self.text6.Hide()
            self.differenceDisplay.Hide()
